...
- [WhatsApp: YCloud → platform flow](docs/whatsapp_ycloud_platform_flow.md) — code-first map of how a WhatsApp message becomes platform state: what YCloud knows (and does not), Coexistence and message echoes, the eight-step inbound path, the agent/observer dispatch fork where tenant identity is decided, the outbound sender model, and known gaps.
- [WhatsApp provider portability assessment](docs/whatsapp_provider_portability_assessment.md) — current provider-neutral boundary, remaining portability gaps, and the recommended global-provider cutover path.
...
- [Simulador financeiro](docs/pages/simulador_financeiro.md) — dedicated what-if workspace for current-month capacity, mix, and occupancy scenarios by location.
...
- [Assistant, sidebar, and admin controls roadmap](docs/ROADMAPS/assistant_sidebar_admin_controls_roadmap_v0.1_2026-09-05.md) — proposed outside-click Lob minimization, bottom-anchored WhatsApp navigation, and platform-admin-only Mock Chat with frontend and API enforcement.
- [First-user onboarding — "Comece aqui" operation setup roadmap](docs/ROADMAPS/first_user_onboarding_operation_setup_roadmap_v0.1_2026-09-06.md) — implemented locally: single `operation_configured` session signal driving a "Comece aqui" badge on Minha Operação, a conditional hoist of that item to the top of the sidebar, a one-time post-login redirect to setup, and an Agenda setup empty state; all clear automatically once a tenant has a Local and a work journey.
...
- [Shared platform AI agent number roadmap](docs/ROADMAPS/shared_platform_agent_number_roadmap_v0.1_2026-09-06.md) — proposed replacement of the per-tenant `agent_phone` with one shared platform agent number in the same YCloud account, inverting agent-channel tenant resolution to sender-keyed, plus a fix for observer/agent lane cross-contamination, an instructor binding handshake, and admin WhatsApp-number management.
...
- [Advisory work journey & estimated simulator roadmap](docs/ROADMAPS/advisory_work_journey_and_estimated_simulator_roadmap_v0.1_2026-08-31.md) — proposed change from hard journey booking boundaries to advisory guidance, plus a Monday–Saturday estimated simulator baseline until the journey is configured.
...
- [Simulator month scope & premises consolidation roadmap](docs/ROADMAPS/simulator_month_scope_and_premises_consolidation_roadmap_v0.1_2026-09-06.md) — implemented locally: fixed `/financeiro/simulador` to the current-month window (inherited period presets removed), consolidated period + location into one top "Premissas da simulação" block, and replaced the price-override lever with a read-only, single-location price matrix.
...
- [Mobile readiness audit (2026-08-31)](docs/mobile_readiness_audit_2026-08-31.md) — code-level assessment, implemented mobile remediation, and final device release gate.
